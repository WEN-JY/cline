...
export async function activate(context: vscode.ExtensionContext) {
	...
	vscode.commands.executeCommand("setContext", "super.isDevMode", IS_DEV && IS_DEV === "true")
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			...
			if (currentVersion !== lastShownPopupNotificationVersion && previousVersion) {
				...
				await vscode.commands.executeCommand("super-agent.SidebarProvider.focus")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.plusButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.mcpButtonClicked", (webview: any) => {
	...
	context.subscriptions.push(vscode.commands.registerCommand("super.popoutButtonClicked", openClineInNewTab))
	context.subscriptions.push(vscode.commands.registerCommand("super.openInNewTab", openClineInNewTab))

	context.subscriptions.push(
		vscode.commands.registerCommand("super.settingsButtonClicked", (webview: any) => {
			const isSidebar = !webview
			const webviewType = isSidebar ? WebviewProviderTypeEnum.SIDEBAR : WebviewProviderTypeEnum.TAB
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.historyButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.accountButtonClicked", (webview: any) => {
	...
	*/
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.addToChat", async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			await vscode.commands.executeCommand("super.focusChatInput") // Ensure Cline is visible and input focused
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.addTerminalOutputToChat", async () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.fixWithCline", async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			// Add this line to focus the chat input first
			await vscode.commands.executeCommand("super.focusChatInput")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.explainCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("super.focusChatInput") // Ensure Cline is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.improveCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("super.focusChatInput") // Ensure Cline is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.focusChatInput", async () => {
			...
			if (activeWebviewProvider?.view && activeWebviewProvider.view.hasOwnProperty("reveal")) {
				...
				await vscode.commands.executeCommand("super-agentt.SidebarProvider.focus")
				...
				if (!activeWebviewProvider) {
					...
					await vscode.commands.executeCommand("super.openInNewTab")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("super.generateGitCommitMessage", async () => {
