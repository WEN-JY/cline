...
`,
	...
	const settings = {
		...
		"workbench.view.extension.qiyefazhan.super-agent-ActivityBar.visible": true,
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.qiyefazhan.super-agent-ActivityBar",
			when: "viewContainer.workbench.view.extension.qiyefazhan.super-agent-ActivityBar.enabled",
		},
		{
			key: "alt+shift+c",
	...
	const args = [
		...
		"--start-up-extension",
		"qiyefazhan.super-agent",
		// Run a command on startup to open Cline
		"--command",
		"workbench.view.extension.qiyefazhan.super-agent-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			...
			require('vscode').commands.executeCommand('workbench.view.extension.qiyefazhan.super-agent-ActivityBar');
			...
			function activate(context) {
				...
				let disposable = vscode.commands.registerCommand('cline-activator.activate', async function () {
					try {
						...
						const extension = vscode.extensions.getExtension('qiyefazhan.super-agent');
						if (!extension) {
						...
						await vscode.commands.executeCommand('workbench.view.extension.qiyefazhan.super-agent-ActivityBar');
